...
function applyFilters(strings: StringProduct[], filterPath: string[]): StringProduct[] {
  ...
  if (filterString.includes("rating-45")) {
    filtered = filtered.filter((s) => s.reviewscore >= 4.5);
  } else if (filterString.includes("rating-40")) {
    filtered = filtered.filter((s) => s.reviewscore >= 4.0);
  } else if (filterString.includes("rating-35")) {
    filtered = filtered.filter((s) => s.reviewscore >= 3.5);
  }

  return filtered;
}

export default function StringsSidebar({ showSort = true }: { showSort?: boolean }) {
  const pathname = usePathname();
  const router = useRouter();
  const allData = stringsData as StringProduct[];
  ...
  const activeFilters = useMemo(() => {
    ...
    const idx = segments.indexOf("strings");
    if (idx === -1) return [];
    return segments
      .slice(idx + 1)
      .filter((s) => s !== "filter")
  ...
  const allPriceRanges = ["under-500", "under-1000", "under-1500"];
  const allRatings = [
  ...
  const buildPath = (filters: string[]) =>
    filters.length ? `/strings/${filters.join("/")}` : "/strings";
