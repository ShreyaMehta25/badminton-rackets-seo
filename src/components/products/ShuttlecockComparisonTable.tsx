    ...
    <>
      ...
      <div className="hidden md:block overflow-x-auto border border-slate-200 bg-white shadow-md rounded-lg">
        <table className="min-w-full text-xs md:text-sm">
          ...
          <tbody>
            {shuttlecocksWithIds.map((shuttlecock, index) => (
              <tr
                key={shuttlecock.id}
                onClick={() => router.push(`/shuttlecock/${shuttlecock.id}`)}
              ...
              >
                ...
                <td className="px-3 md:px-6 py-4 md:py-5 text-center">
                  <span className="px-2 md:px-3 py-0.5 md:py-1 text-xs rounded-md bg-green-50 text-green-700 border border-green-200 font-medium">
                    {shuttlecock.speed}
                  </span>
                </td>
