...
export default function GripsComparisonTable({ grips }: { grips: Grip[] }) {
  const router = useRouter();
  const gripsWithIds = addIdsToProducts(grips);
    ...
    <>
      ...
      <div className="hidden md:block overflow-x-auto border border-slate-200 bg-white shadow-md rounded-lg">
        <table className="min-w-full text-xs md:text-sm">
          ...
          <tbody>
            {gripsWithIds.map((grip, index) => (
              <tr
                key={grip.id}
                onClick={() => router.push(`/grips/${grip.id}`)}
