...
export default function RacketsLayout({
  ...
  // Hide navbar on racket detail pages
  // Detail pages are /rackets/[id] where id doesn't match filter patterns
  const isDetailPage = pathname?.match(/^\/rackets\/[^/]+$/) && 
    !pathname?.includes("for-") && 
    !pathname?.includes("under-") && 
    !pathname?.match(/\/(brand|4u|3u|5u)(-|$)/);

  return (
    <>
      {/* Fixed navbar - hidden on detail pages */}
      {!isDetailPage && (
      ...
      <div className={`max-w-7xl mx-auto px-6 ${isDetailPage ? "pt-10" : "pt-20"} pb-10`}>
        {/* Dynamic grid - only show sidebar on non-detail pages */}
        {!isDetailPage ? (
          <div
